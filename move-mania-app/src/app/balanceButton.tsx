...
export default function BalanceButton() {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn && userInfo) {
      // setAccount(user);
      getBalance(userInfo.address, `${process.env.MODULE_ADDRESS}::g_move::GMOVE`).then((balance) => {
        setBalance(balance);
      });
    }
    //     });
  ...
  const onWithdraw = async () => {
    ...
    const tx = await transferApt(keylessAccount, parseFloat(transferAmount), recipientAddress, `${process.env.MODULE_ADDRESS}::g_move::GMOVE`);
    ...
    toast({
      ...
      description: <Link href={`https://blue.explorer.movementlabs.xyz/txn/${tx.version}/?network=testnet`} target="_blank" className="underline">View transaction</Link>
    })
    setLoading(false);
    ...
    return (
      <Dialog>
        <DialogTrigger asChild>
          <button className="bg-neutral-800 hover:bg-neutral-700 px-2 py-1 text-xs text-white font-semibold">
            {balance?.toFixed(2) || parseInt('0').toFixed(2)} GMOVE
          </button>
        </DialogTrigger>
        <DialogContent className="bg-neutral-950">
          <input type="text" autoFocus className="hidden" />
          <DialogTitle>Your balance: <span className="font-normal">{balance?.toFixed(2) || parseInt('0').toFixed(2)} GMOVE</span></DialogTitle>
          <DialogTitle>Deposit Funds</DialogTitle>
          <DialogDescription>
            Send MOVE to the public address below to deposit funds into your account.
          </DialogDescription>
          ...
          <DialogDescription>
            Withdraw MOVE from your account to the address specified below.
          </DialogDescription>
        ...
        <div>
          ...
          <DialogDescription>
            Purchase MOVE with your credit card and deposit it into your account.
          </DialogDescription>
        </DialogContent>
      </Dialog>
    )
