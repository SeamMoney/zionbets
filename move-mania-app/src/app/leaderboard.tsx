...
export default function Leaderboard() {
    ...
    const getBalances = async (users: { username: string, public_address: string, private_key: string }[]) => {
        const balances = await Promise.all(
            users.map(async (user) => {
                try {
                    const balance = await getBalance(user.private_key, `${process.env.MODULE_ADDRESS}::g_move::GMOVE`);
                    return {
                        username: user.username,
                        public_address: user.public_address,
                        gmove_balance: balance,
                    };
                } catch (e) {
                    console.error(e);
                    return {
    ...
    return (
        <div className="border border-neutral-700 h-full flex flex-col items-left gap-2 w-full min-h-[200px] max-h-[700px]">
            <span className="font-semibold text-lg pt-1 ps-4 flex flex-row items-center"><span>GMOVE Leaderboard</span><Link href='https://x.com/zionbets/status/1770459737076953351?s=20' target="_blank"><InfoIcon className="h-4 opacity-50 hover:opacity-100 hover:cursor-pointer" /></Link></span>
                ...
                <thead className="">
                    <tr className="border-b border-neutral-800 text-neutral-400">
                        ...
                        <th className="w-[100px] text-right pr-4">
                            Balance{" "}
                            <span className="text-neutral-500  text-xs">gmove</span>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    {users
                        .sort((a, b) => {
                            return b.gmove_balance - a.gmove_balance;
                        })
                        .slice(0, 10)
                        .map((player, index) => (
                            <tr key={index} className="text-white text-sm  h-8">
                                ...
                                <td className="text-right pr-4 w-[100px] text-right pr-4  bg-neutral-800/40 bg-[#264234]/40 border-b border-neutral-800">
                                    {player.gmove_balance.toLocaleString(undefined, {
